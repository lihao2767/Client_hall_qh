...
var RoomCfgManager = app.BaseClass.extend({
    ...
    GetRoomData:function(gameType,cfg){//gameType大写
        ...
        if('nn' == roomData.bigName1){
            ...
            if(1 == cfg.sign){
                roomData.smallName0 = '庄家十三张';
                roomData.smallName1 = 'sss_zz';
            }
            else if(2 == cfg.sign){
                roomData.smallName0 = '通用十三张';
    ...
    GetWanFaFormServerBatchAdd:function(gameIdList){
        //清空请求的ID重新获取
        let addGameIdList=new Array();
        for(let i=0;i<gameIdList.length;i++){
            let gameString=app.ShareDefine().GametTypeID2PinYin[gameIdList[i]];
            if(!this.gameCreateCfgAll[gameString]){
                this.gameCreateCfgAll[gameString]=new Array();
                addGameIdList.push(gameIdList[i]);
            }
        }
        let self = this;
        if(addGameIdList.length>0){
            //根据选择的城市向服务端请求所有游戏
            app.NetManager().SendPack("game.CGetGameCreateByGameList", {"gameIdList": addGameIdList}, function (event) {
                for(let i=0;i<event.length;i++){
                    let item=event[i];
                    let gamety=item.GameName;
                    self.gameCreateCfgAll[gamety].push(item);
                }
            }, function (event) {
                //app.SysNotifyManager().ShowSysMsg("批量获取玩法失败，请联系客服", [], 3);
            });
        }
    },
    GetWanFaFormServerBatch:function(gameIdList){
        //清空请求的ID重新获取
        for(let i=0;i<gameIdList.length;i++){
            let gameType=app.ShareDefine().GametTypeID2PinYin[gameIdList[i]];
            this.gameCreateCfgAll[gameType]=new Array();
        }
        let self = this;
        if(gameIdList.length>0){
            //根据选择的城市向服务端请求所有游戏
            app.NetManager().SendPack("game.CGetGameCreateByGameList", {"gameIdList": gameIdList}, function (event) {
                for(let i=0;i<event.length;i++){
                    let item=event[i];
                    let gamety=item.GameName;
                    self.gameCreateCfgAll[gamety].push(item);
    ...
    WanFaFormServer:function(gameType,cfg,setlabel,maxlength=0,addString="",setWanFa=null){
        //return;  //服务端会报错，不处理
        let type=typeof(gameType);
        let gameId=0;
        if(type== 'number'){
            ...
            gameId=app.ShareDefine().GametTypeNameDict[gameType.toUpperCase()];
        }

        let gameString=app.ShareDefine().GametTypeID2PinYin[gameId];

        if(this.gameCreateCfgAll[gameString]){
            if(this.gameCreateCfgAll[gameString].length>0){
                let allString=addString+this.WanFa(gameType,cfg);
                if(maxlength==0){
                    setlabel.string=allString;
                }else{
                    setlabel.string=allString.substr(0,maxlength);
                }
                if(setWanFa){
                    setWanFa.wanfa=allString;
                }
                return;
            }
        }
        ...
        app.NetManager().SendPack("game.CGetGameCreateByGameId", {"gameId": gameId}, function (event) {
                    self.gameCreateCfgAll[gameString]=event;
                    let allString=addString+self.WanFa(gameType,cfg);
                    if(maxlength==0){
                        setlabel.string=allString;
                    }else{
                        setlabel.string=allString.substr(0,maxlength);
    ...
    WanFa:function(gameType,cfg){//gameType大写
        ...
        let gameCreateCfg=this.gameCreateCfgAll[gameType.toLowerCase()];
        ...
        for(let key in gameCreateCfg){
            let gameName = gameCreateCfg[key].GameName;
            let isWanFa = gameCreateCfg[key].isWanFa;
            if(gameType == gameName && 1 == isWanFa){
                let dataKey = gameCreateCfg[key].Key;
                let value = cfg[dataKey];
                if(dataKey == "kexuanwanfa"){
                    if(gameType == 'pdk' || gameType == 'lypdk' || gameType == 'lcpdk' ||gameType == 'qjpdk' || gameType == 'jtpdk' || gameType == 'gapdk' || gameType == 'japdk' ||gameType == 'jxndpdk' || gameType == 'yxpdk' || gameType == 'ygpdk' || gameType == 'xgpdk' || gameType == 'hhpdk' || gameType == 'yzpdk' || gameType == 'xyxxpdk'){
                ...
                let ToggleType = gameCreateCfg[key].ToggleType;
                let ToggleDesc = gameCreateCfg[key].ToggleDesc.split(',');
                if(0 == ToggleType){
                    ...
                    for(let j = 0; j < value.length;j++){
                        ...
                        if(dataKey=="kexuanwanfa"){
                            ...
                            }else if (gameType == 'lppdk' || gameType == 'pdk' || gameType == 'lcpdk' ||gameType == 'qjpdk' || gameType == 'gapdk' || gameType == 'japdk'|| gameType == 'yxpdk' || gameType == 'wxpdk' || gameType == 'lypdk') {
                            ...
                            }else if (gameType == 'jtpdk' || gameType == 'tzpdk' || gameType == 'xcpdk' || gameType == 'zxpdk') {
                                if (value[j] == 13 || value[j] == 14 || value[j] == 15 || value[j] == 18) {
                                    continue;
                                }
                            }else if (gameType == 'jxfzpdk') {
                                if (value[j] == 13 || value[j] == 15 || value[j] == 22) {
                                    continue;
                                }
                            ...
                            }else if(gameType == 'zxpdk' || gameType == 'pdk' || gameType == 'lypdk' || gameType == 'lcpdk' || gameType == 'gapdk' || gameType == 'japdk' || gameType == 'yxpdk' || gameType == 'hspdk' || gameType == 'qjpdk' || gameType == 'wxpdk'){
                                if(value[j] == 14){
                                    continue;
                                }
                                if (value[j] == 11) {
                    ...
                    if(dataKey=="difen"){
                        wanfaStr = wanfaStr + "，底分:" + value;
                    } else if(dataKey=="jiesanfafen"){
                        wanfaStr = wanfaStr + "，解散罚分:" + value;
                    } else{
                        if(gameType == "cbdg" && dataKey == "tuoguanshufenfenzhi"){
                            if(value != -1){
                                wanfaStr = wanfaStr + "," + value;
                            }
                        } else {
                            wanfaStr = wanfaStr + "," + value;
                        }
