...
cc.Class({
    ...
    OnShow: function (roomId, playerAll, gameType, unionId = 0, roomKey = null, defaultPage = 1) {
        ...
        let gameNameList = ["bw", "pypp", "ctwsk", "ygwsk", "qcdg", "tsdg", "dhd", "ysdz", "wsk", "sdh", "cdp", "lkwsk",
            ...
            "jayxddz", "gast", "pysft", "thgz", "bzp", "tgwsk", "sys", "dzzj", "pydd", "lczp", "qjwsk", "qsj", "xswsk",
            "jhwpk"];
        //特殊处理 包王垂直布局
        ...
        let verticalScrollviewGameNames = ["jayxddz","jsch"];
    ...
    ShowPX6GTDiPai: function (setEnd) {
        ...
        //------------------肇庆麻将
        if (app.ShareDefine().GametTypeNameDict[this.gameType.toUpperCase()] == app.ShareDefine().GameType_ZQMJ) {
            let maList = setEnd["maList"];
            let zhongList = setEnd["zhongList"];
            this.maListNode.active = !(zhongList.length <= 0 && maList.length <= 0);
            this.maListNode.scale = 0.55;
        ...
        //------------------南宁麻将
        if (app.ShareDefine().GametTypeNameDict[this.gameType.toUpperCase()] == app.ShareDefine().GameType_NNMJ) {
            let maList = setEnd["maList"];
            let zhongList = setEnd["zhongList"];
            this.maListNode.active = !(zhongList.length <= 0 && maList.length <= 0);
            this.maListNode.scale = 0.6;
            for (let i = 0; i < this.maListNode.children.length; i++) {
                this.maListNode.getChildByName('card0' + (i + 1)).active = false;
                this.maListNode.getChildByName("card0" + (i + 1)).color = cc.color(255, 255, 255);
            }
        ...
        //------------------南丰麻将
        if (app.ShareDefine().GametTypeNameDict[this.gameType.toUpperCase()] == app.ShareDefine().GameType_NFMJ) {
            let maList = setEnd["maList"];
            let zhongList = setEnd["zhongList"];
            this.maListNode.active = !(zhongList.length <= 0 && maList.length <= 0);
            this.maListNode.scale = 0.6;
