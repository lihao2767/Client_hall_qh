...
cc.Class({
    ...
    GetTuiGuangUrl:function(){
        ...
        if(this.TuiGuangUrl == ''){
            ...
            let long_url="http://api.t.sina.com.cn/short_url/shorten.json?source=3271760578&url_long=https://8.159.141.75:88/"+ShortHeroID+'/';
    ...
    BeginShare:function(roomKey,text){
        ...
        this.roomKey=roomKey;
        //校服
        let long_url="http://api.t.sina.com.cn/short_url/shorten.json?source=3271760578&url_long=https://8.159.141.75:88/room"+roomKey;
    ...
    SuccessTcnUrl:function(serverPack){
        let shareUrl="https://8.159.141.75:88/"+app.ComTool().GetPid(app.HeroManager().GetHeroProperty("pid"))+"/";
        if(this.TuiGuangUrl){
            text=this.ShareText+"\n"+"下载地址:"+shareUrl;
        }else{
            if(url_short){
