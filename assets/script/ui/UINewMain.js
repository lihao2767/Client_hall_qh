...
cc.Class({
    ...
    properties: {
        ...
        mjSprite:[cc.SpriteFrame],
        pkSprite:[cc.SpriteFrame],
        zpSprite:[cc.SpriteFrame],
    },
               ...
               //    version="1.1.1";
                   if(version){
                       ...
                       if(AccountActive>1000){
                          ...
                          this.IOSUrl="http://8.159.141.75:88/mygame_ios.php?game=hall_split&pid="+app.HeroManager().GetHeroProperty("pid");
    ...
    OnShow: function () {
        ...
        //获取后台开关是否需要检测子游戏热更
        this.CheckHotUpdate="http://8.159.141.75:88/index.php?module=Api&action=OnlineStatus";
        ...
        if (this.allSelectCityData.length == 0 ||  this.isFirstLogin()==true) {
            ...
            if (!this.selectCityConfig) {
                ...
                app.Client.GetGameListByCityId(this.allSelectCityData[0]['selcetId']);
            }
        }
        ...
        if (cc.sys.isNative) {
            this.ChouJiangUrl='http://8.159.141.75:88/index.php?module=Api&action=GetLuckGoods';
            this.WenJuanUrl="http://8.159.141.75:88/mu?module=Api&action=GetQuestionnaire&pid="+heroID;
            this.IpUrl='http://8.159.141.75:88/myip.php';
            this.KeFuUrl = "http://8.159.141.75:88/index.php?module=Api&action=Contact";
        }else{
            this.ChouJiangUrl='http://8.159.141.75:88/index.php?module=Api&action=GetLuckGoods';
            this.WenJuanUrl="http://8.159.141.75:88/mu?module=Api&action=GetQuestionnaire&pid="+heroID;
            this.IpUrl='http://8.159.141.75:88/myip.php';
        ...
        this.ReSetPoint();//检查节点归位

        //手机登陆如果没有设置头像跟昵称的弹出
        this.CheckMobileHeadName();
    },
    CheckMobileHeadName:function(){
        let isMobile = app.HeroAccountManager().GetAccountProperty("isMobile");
        if(isMobile==1){    
    ...
    ReSetPoint:function(){
        ...
        if(AccessPoint>0){
            let serverIP = app.Client.GetBaseGameServerIP();
            let BaseServerIP=serverIP;
            let AccountActive = app.LocalDataManager().GetConfigProperty("Account", "AccountActive");
            ...
            if(myCityID){
                ...
                if(CityPoints){
                    ...
                    if(CityPointsList.indexOf(myCityID)>-1){
                        //用户选择的城市有配置独立的节点
                        if(AccountActive>10){
                            serverIP='h'+myCityID+'.'+serverIP; //活跃用户
                        }else{
                            serverIP='c'+myCityID+'.'+serverIP; //普通用户
                        }
                    }
                }
            }
            //没有城市独立节点，走本身分层节点
            if(BaseServerIP==serverIP){
                if(AccountActive>200){
                    serverIP='0200.'+serverIP;
                }else if(AccountActive>100){
            ...
            //检测玩家节点是否可以打开，可以打开的话归位节点（已注释，不访问checklive.php）
            // this.CheckServerUrl = ['http://',serverIP, ":","88", "/checklive.php"].join("");
            // console.log("ReSetPoint CheckServerUrl:"+this.CheckServerUrl);
            // this.SendHttpRequest(this.CheckServerUrl, "","GET",{});
        }
    },
    ...
    Event_ShowGameListByLocation:function(event){
        ...
        for (var i = 0; i < gameList.length; i++) {
            ...
            let gameNm=this.gametypeConfig[gameList[i]]["Name"];
            let gameNm1=this.gametypeConfig[gameList[i]]["Name_1"];
            let gameTp= this.gametypeConfig[gameList[i]]["Type"];
            let yushu=gameList[i]%3;
            
            if(yushu==0){
                btn_game.getChildByName("lb_name").getComponent(cc.LabelOutline).color = new cc.Color(5, 144, 61);
            }else if(yushu==1){
                btn_game.getChildByName("lb_name").getComponent(cc.LabelOutline).color = new cc.Color(158, 96, 6);
            }else{
                btn_game.getChildByName("lb_name").getComponent(cc.LabelOutline).color = new cc.Color(146, 107, 3);
            }

            btn_game.getChildByName("lb_name").getComponent(cc.Label).string=gameNm1;
            if(gameTp==1){
                //麻将
                if(gameNm.indexOf('mj')){
                    btn_game.getComponent(cc.Sprite).spriteFrame = this.mjSprite[yushu];
                }else{
                    btn_game.getComponent(cc.Sprite).spriteFrame = this.zpSprite[yushu];
                }
            }else if(gameTp==2){
                //扑克
                btn_game.getComponent(cc.Sprite).spriteFrame = this.pkSprite[yushu];
            }
    ...
    Event_HeroProperty:function (event) {
        ...
        if(argDict["Property"] == "gold"){
            ...
            if(typeof(argDict["name"])!="undefined"){
                this.lb_heroName.string=argDict["name"];
            }
        }else if(argDict["Property"]=="headimg"){
            console.log("Event_HeroProperty change headimg:"+argDict["name"]);
            let heroID = app.HeroManager().GetHeroProperty("pid");
            this.WeChatManager.InitHeroHeadImage(heroID,argDict["name"],true);
            this.WeChatHeadImage1.OnLoad();
            this.WeChatHeadImage1.ShowHeroHead(heroID,argDict["name"]);
        }
    },
    ...
    OnReceiveHttpPack:function(serverUrl, httpResText){
        try{
            ...
            else if(serverUrl==this.CheckServerUrl){
                if(serverPack["IsSuccess"] == 1){
                    console.log("ReSetPoint Point ReSet");
                    app.LocalDataManager().SetConfigProperty("Account", "AccessPoint",0); //已经没被攻击，节点归位
                }
            }
    ...
    InitGameBtnList:function(serverPack){
        this.FormManager.ShowForm("UICreatRoomNew",serverPack,this.gameName);
    },
    OpenClub:function(){
    ...
    OnClick:function(btnName, btnNode){
        if('btn_club' == btnName){
            ...
            this.FormManager.ShowForm("UIMoreGame",null,null,this.gameList);
        }
            ...
            this.node.getChildByName('bottom').getChildByName('left').getChildByName('layout').active=!this.node.getChildByName('bottom').getChildByName('left').getChildByName('layout').active;
             ...
             cc.sys.openURL("http://8.159.141.75:88/index.php?module=Report&action=Index&playerID="+app.HeroManager().GetHeroID());
           ...
           this.FormManager.ShowForm("UIBangDingTuiGuang");
            ...
            cc.sys.openURL("http://8.159.141.75:88/feedback/"+app.HeroManager().GetHeroID()+"/"+myCityID+"/?key="+sign);
        }
            ...
            this.FormManager.ShowForm("UICreatRoomNew",{"gameList":app.Client.GetAllGameId()},this.gameName);
