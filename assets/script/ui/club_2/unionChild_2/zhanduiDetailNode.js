...
cc.Class({
    ...
    UpdateScrollView:function(serverPack, isRefresh){
    	...
    	for (let i = 0; i < serverPack.clubTeamListInfoList.length; i++) {
            ...
            child.getChildByName("lb_eliminatePoint").getComponent(cc.Label).string = matchItem.gameTicket;
            child.getChildByName("lb_consume").getComponent(cc.Label).string = matchItem.consume;
            if (matchItem.position == 2) {
                //推广员不显示调出按钮
                child.getChildByName("btn_detail").active = false;
            }else{
                child.getChildByName("btn_detail").active = true;
            }
    		child.active = true;
    ...
    OnClick:function(btnName, btnNode){
        if ("btn_timeDemo" == btnName) {
            ...
            app.FormManager().ShowForm('ui/club_2/UIPromoterPlayerInfo',this.clubId,btnNode.parent.playerData.pid,this.curType);
