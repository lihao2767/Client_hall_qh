...
cc.Class({
    ...
    //--------------显示函数-----------------
    OnShow:function(clubId, unionId, unionName, unionPostType, myisminister, unionSign, levelPromotion,clubData,btn_defaultName="btn_newRaceRankZhongzhi"){
        ...
        this.isMinisetrZhongZhi=clubData.isMinisetrZhongZhi;
        this.unionName = unionName;
        this.unionSign = unionSign;
        this.levelPromotion = levelPromotion;
        this.UpdateLeftBtn();

        if(this.clubData.matchPower==0 && this.unionPostType == app.ClubManager().UNION_GENERAL){
            btn_defaultName = "btn_myrace";
        }else if (this.unionPostType == app.ClubManager().UNION_GENERAL && this.levelPromotion > 0) {//普通成员
    ...
    UpdateLeftBtn:function(){
        ...
        let btn_MemberManageZhongzhi = this.node.getChildByName("left").getChildByName("btn_MemberManageZhongzhi");
        let btn_myrace = this.node.getChildByName("left").getChildByName("btn_myrace");
        ...
        if(this.myisminister == app.ClubManager().Club_MINISTER_MGRSS){//赛事管理
            ...
            btn_MemberManageZhongzhi.active=true;
        }else if (this.unionPostType == app.ClubManager().UNION_GENERAL) {//普通成员

            if(this.clubData.matchPower>0){
                btn_RaceManageZhongzhi.active = true;
                btn_newRaceRankZhongzhi.active = true;
                btn_MemberManageZhongzhi.active=true;
            }else{
                ...
                btn_MemberManageZhongzhi.active=false;
    ...
    ClickLeftBtn:function(clickName){
        ...
        for (let i = 0; i < allRightBtn.length; i++) {
            if (allRightBtn[i].name == clickName + "Node") {
                ...
                allRightBtn[i].getComponent(allRightBtn[i].name).InitData(this.clubId, this.unionId, this.unionPostType, this.myisminister, this.unionName, this.unionSign, this.levelPromotion,this.isMinisetrZhongZhi);
            }else{
    ...
    OnClick:function(btnName, btnNode){
        if('btn_close'==btnName){
            ...
            this.ClickLeftBtn(btnName);
        }else if('btn_myrace'==btnName){
            this.ClickLeftBtn(btnName);
        }else if('btn_MemberManageZhongzhi'==btnName){
            this.ClickLeftBtn(btnName);
        }
    },
});