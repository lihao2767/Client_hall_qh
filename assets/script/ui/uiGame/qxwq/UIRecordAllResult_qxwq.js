...
cc.Class({
    ...
    OnCreateInit: function () {
        ...
        //加载麻将资源，防止异步坑爹的bug
        // this.LoadAllImages();
		this.RegEvent("qxwq_ChangeSelectState", this.Event_ChangeSelectState, this);
    },
    ...
    OnShow: function (roomId, playerAll, gameType, unionId = 0, roomKey = null, defaultPage = 1) {
        ...
        this.Players = this.PokerPlayers;
        this.getWndNodePath = "PokerScrollView/view/PokerPlayers";
        this.getWndNodePath = "horizontalPlayers";
        // console.log("-------------------cp")
        // this.getWndNodePath = "MJPlayers";
        //动态生成节点
	...
	Event_ChangeSelectState: function (targetPos) {
		this.ShowAllWater(targetPos);
	},
    
	ShowAllWater: function (pos) {
		let historyCItemList = this.posResultList[pos].historyCItemList;

		this.waterContents.removeAllChildren();
		for (let idx = 0; idx < historyCItemList.length; idx++) {
			let waterInfo = historyCItemList[idx];
			let waterNode = cc.instantiate(this.waterItem);
			this.waterContents.addChild(waterNode);
			waterNode.getChildByName("lb_pos").getComponent(cc.Label).string = waterInfo.posID + 1;

			let huTypes = [];
			let huTypeDict = this.GetHuTypeDict();
			for (let idx = 0; idx < waterInfo.huTypeList.length; idx++) {
				let huType = waterInfo.huTypeList[idx];
				if (!Object.hasOwnProperty.call(huTypeDict, huType)) {
					console.error(`胡类型 ${huType} is not exist!`);
				}
				huTypes.push(huTypeDict[huType]);
			}

			waterNode.getChildByName("scrollView").getChildByName("lb_huType").getComponent(cc.Label).string = huTypes.join("、");
            waterNode.getChildByName("lb_point").getComponent(cc.Label).string = this.ToUiScore(waterInfo.point);
			// waterNode.getChildByName("lb_score").getComponent(cc.Label).string = waterInfo.winPoit;
            if (typeof (waterInfo.sportsPoint) != "undefined") {
                waterNode.getChildByName("lb_sportsPoint").getComponent(cc.Label).string = "赛:" + this.ToUiScore(waterInfo.sportsPoint);
            } else {
                waterNode.getChildByName("lb_sportsPoint").getComponent(cc.Label).string = "";
            }

			let color = waterInfo.winPosID == pos ? new cc.Color().fromHEX("#80ff48") : new cc.Color().fromHEX("#ffe24d");
			waterNode.getChildByName("lb_pos").color = color;
			waterNode.getChildByName("scrollView").getChildByName("lb_huType").color = color;
			waterNode.getChildByName("lb_point").color = color;
            waterNode.getChildByName("lb_sportsPoint").color = color;
		}
	},
	...
	GetHuTypeDict: function () {
		let huTypeDict = {};
		huTypeDict["LiangYi"]="两仪";
        huTypeDict["SiXiang"]="四象";
        huTypeDict["BaGua"]="合卦";
        huTypeDict["TaiJiBaGua"]="太极八卦";
        huTypeDict["QiuFengSaoLuoYe"]="秋风扫落叶";
        huTypeDict["LiYuYueLongMen"]="鲤鱼跃龙门";
        huTypeDict["DingHaiShenZhen"]="定海神针";
        huTypeDict["RuYiJinGuBang"]="如意金箍棒";
        huTypeDict["TianNvSanHua"]="天女散花";
        huTypeDict["DanZhaDanPaiXing"]="太极八卦";
        huTypeDict["ShuangPaoQiXiang"]="双炮齐响";
        huTypeDict["SanYangKaiTai"]="三阳开泰";
        huTypeDict["BaiHuaQiFang"]="百花齐放";
        huTypeDict["FengWuJiuTian"]="凤舞九天";
        huTypeDict["BaiHuXiaShan"]="白虎下山";
        huTypeDict["LiuYueFeiXue"]="六月飞雪";
        huTypeDict["ShengShengShiShi"]="生生世世";
        huTypeDict["BiYiQiFei"]="比翼齐飞";
        huTypeDict["YiHuaKaiTian"]="一画开天";
        huTypeDict["BaXianGuoHai"]="八仙过海";
        huTypeDict["YuZhouHongHuang"]="宇宙洪荒";
        huTypeDict["ErLongXiZhu"]="二龙戏珠";
        huTypeDict["ZhuQueLingWu"]="朱雀翎舞";
        huTypeDict["TianYaHaiJiao"]="天涯海角";
        huTypeDict["ShanShanXingGuang"]="闪闪星光";
        huTypeDict["XuanWuChuHai"]="玄武出海";
        huTypeDict["ZhanLongZaiYe"]="战龙在野";
        huTypeDict["TianDiSiLing"]="天地四灵"; 
    ...
    ShowData: function () {
        ...
        this.posResultList = data.posResultList;
		// this.ShowAllWater(this.posResultList[0].pos);
		app.Client.OnEvent("qxwq_ChangeSelectState", this.posResultList[0].pos);
