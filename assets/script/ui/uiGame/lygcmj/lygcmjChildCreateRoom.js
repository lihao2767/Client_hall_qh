...
var mzmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
		...
		let gangdifen=-1;
		if (!this.GetEditBoxByKey('gangdifen')) {
			app.SysNotifyManager().ShowSysMsg('请检查输入的区间值是否为1-50整数');
			return;
		}else{
			gangdifen=this.GetEditBoxByKey('gangdifen');
		}
		let cidifen=-1;
		if (!this.GetEditBoxByKey('cidifen')) {
			app.SysNotifyManager().ShowSysMsg('请检查输入的区间值是否为1-50整数');
			return;
		}else{
			cidifen=this.GetEditBoxByKey('cidifen');
		}
		let difen=-1;
