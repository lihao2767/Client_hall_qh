...
var thgjmjChildCreateRoom = cc.Class({
    ...
    CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
        let sendPack = {};
        let difen=-1;
        if (!this.GetEditBoxByKey('difen')) {
            app.SysNotifyManager().ShowSysMsg('请检查输入的区间值是否正确');
            return false;
        }else{
            difen=this.GetEditBoxByKey('difen');
        }
    ...
    OnToggleClick:function(event){
        ...
        if('jushu' == key || 'renshu' == key || 'fangfei' == key){
            ...
            if(this.Toggles['hupai'][2].getChildByName('checkmark').active){
            ...
            if (toggleIndex == 6 && !this.Toggles['hupai'][2].getChildByName('checkmark').active) {
                app.SysNotifyManager().ShowSysMsg("勾选新计分才可以选",[],3);
                return;
            }

            if (toggleIndex == 5 && this.Toggles['hupai'][2].getChildByName('checkmark').active) {
                app.SysNotifyManager().ShowSysMsg("新计分不可以选",[],3);
                return;
            }
        }
    ...
    GetEditBoxByKey: function (key) {
        ...
        let reg = /^[0-9]\d*$/;
        for (let i = 0; i < this.Toggles[key].length; i++) {
            let str= this.Toggles[key][i].getChildByName('editboxEX').getComponent(cc.EditBox).string;
            if (!reg.test(str)) {
                return false;
            }
