...
cc.Class({
    ...
    UpdatePlayData: function (PlayerNode, HuList, PlayerInfo, jin1 = 0, jin2 = 0, zhuaNiaoList = null) {
        ...
        this.ShowPlayerNiaoPai(PlayerNode.getChildByName('zhongma'), HuList.maList || [], HuList.zhongList || [], HuList.huType);
    ...
    ShowPlayerNiaoPai: function (ShowNode, maList, zhuaNiaoList, huType) {
        ...
        for (let i = 0; i < zhuaNiaoList.length; i++) {
            zhongListType.push(Math.floor(zhuaNiaoList[i] / 100));
        }
        for (let i = 0; i < maList.length; i++) {
            let cardType = maList[i];
            let node = ShowNode.getChildByName("card" + (i + 1));
            this.ShowImage(node, 'EatCard_Self_', Math.floor(maList[i] / 100));
            node.active = true;
            //更改为没中码都显示码牌
            if (zhongListType.indexOf(Math.floor(cardType / 100)) > -1) {
