...
var fzmjChildCreateRoom = cc.Class({
	...
	OnToggleClick: function (event) {
		...
		if ('jushu' == key || 'renshu' == key || 'fangfei' == key) {
			...
			if(toggleIndex == 2){
				if(this.Toggles['fangjian'][2].getChildByName('checkmark').active==false){
					this.Toggles['fangjian'][4].getChildByName('checkmark').active=false;
					this.UpdateLabelColor(this.Toggles['fangjian'][0].parent);
				}
			}
			if(toggleIndex == 4){
				if(this.Toggles['fangjian'][4].getChildByName('checkmark').active==false){
	...
	UpdateTogglesLabel: function (TogglesNode, isResetPos = true) {
        ...
        if (this.Toggles["gaoji"]) {
            ...
            for (let i = 0; i < this.Toggles["kexuanwanfa"].length; i++) {
                ...
                if (ToggleDesc == "比赛分不能低于0" || ToggleDesc == "只赢当前分") {
                    if (!this.clubData && !this.unionData) {
