...
var jsnyzmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
		...
		let mapaiguize = this.GetIdxsByKey('mapaiguize');
	...
	UpdateOnClickToggle: function (key, toggleIndex) {
		if (this.Toggles["guipai"] && this.Toggles["paixing"]) {
			if (this.Toggles["guipai"][0].getChildByName("checkmark").active) {
				this.Toggles['paixing'][8].getChildByName("checkmark").active = false;
				this.Toggles['paixing'][8].getChildByName("label").color = cc.color(180, 180, 180);
			} else {
				this.Toggles['paixing'][8].getChildByName("label").color = cc.color(158, 49, 16);
