...
var czmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
        ...
        if (!this.GetEditBoxByKey('difen')) {
            app.SysNotifyManager().ShowSysMsg('请检查输入的区间值是否为1-10整数');
            return;
        }else{
	...
	OnToggleClick:function(event){
        ...
        if('jushu' == key || 'renshu' == key || 'fangfei' == key){
			...
			//4,7
			if(toggleIndex == 4){
				let check = this.Toggles['gaoji'][4].getChildByName('checkmark').active;
				if(check==false){
					this.Toggles['gaoji'][7].getChildByName('checkmark').active=false;
					this.UpdateLabelColor(this.Toggles['gaoji'][0].parent);
				}
			}
			if(toggleIndex == 7){
				let check2 = this.Toggles['gaoji'][7].getChildByName('checkmark').active;
				if(check2==false){
