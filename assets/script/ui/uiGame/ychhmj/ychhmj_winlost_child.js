...
cc.Class({
	...
	UpdatePlayData: function (PlayerNode, HuList, PlayerInfo, jin1 = 0, jin2 = 0, jinJin = 0, zhuaNiaoList) {
		...
		this.ShowPlayerDownCard(PlayerNode.getChildByName('downcard'), HuList.publicCardList, HuList.pengCardList, jin1, jin2);
	...
	ShowPlayerInfo: function (ShowNode, PlayerInfo, HuList) {
        ...
        let isTuoDi = HuList["tuoDi"] || false;
        let isQuanJiao = HuList["quanJiao"] || false;
        ShowNode.getChildByName('img_tuodi').active = isTuoDi;
        ShowNode.getChildByName('img_quanjiao').active = isQuanJiao;
    },
	ShowOtherScore: function(ShowNode, huInfo) {
		let hufen = huInfo["huPoint"] || 0;
        ShowNode.getChildByName('hufenLb').getComponent("cc.Label").string = "胡牌分:" + this.ToUiScore(hufen);

        let gangFen = huInfo["gangFen"] || 0;
        ShowNode.getChildByName('gangfenLb').getComponent("cc.Label").string = "杠 分:" + this.ToUiScore(gangFen);
		let bangMaFen = huInfo["bangMaFen"] || 0;
        ShowNode.getChildByName('bangMaFenLb').getComponent("cc.Label").string = "绑马分:" + this.ToUiScore(bangMaFen);
		let maFen = huInfo["maFen"] || 0;
        ShowNode.getChildByName('maFenLb').getComponent("cc.Label").string = "码 分:" + this.ToUiScore(maFen);
    },
	...
	IsShowMulti2: function (huType) {
		let multi2 = ["AnGang", "Gang", "JieGang", "MingGang"];
		let isShow = multi2.indexOf(huType) != -1;
		return isShow;
	},
    ...
    ShowPlayerDownCard: function (ShowNode, publishcard, pengCardList, jin1 = 0, jin2 = 0) {
        ShowNode.active = 1;
        let UICard_DownCard = ShowNode.getComponent("UIMJCard_Down");
        UICard_DownCard.ShowDownCardByZJYYMJ(publishcard, pengCardList, jin1, jin2);
    },
});
