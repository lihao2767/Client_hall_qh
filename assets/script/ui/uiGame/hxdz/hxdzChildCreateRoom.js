...
var sjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
		let sendPack = {};
		let jizhongyueliang = this.GetIdxByKey('jizhongyueliang');
		...
		sendPack = {
			"jizhongyueliang": jizhongyueliang,
			"suanfen": suanfen,
					...
					if ('renshu' == curKey || 'fangfei' == curKey || 'jushu' == curKey) {
						...
						if (this.Toggles['jushu']) {
							...
							for (let i = 0; i < curCostData.length; i++) {
								...
								this.Toggles['jushu'][i].getChildByName('label').getComponent(cc.Label).string = (curCostData[i].SetCount - 700) * 100 + '分';
							}
						}
