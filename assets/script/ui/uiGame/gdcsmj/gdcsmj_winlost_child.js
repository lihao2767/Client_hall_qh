...
cc.Class({
	...
	ShowOtherScore: function(ShowNode, huInfo) {
        ...
        ShowNode.getChildByName('genzhuangfenLb').getComponent("cc.Label").string = "跟庄分" + this.ToUiScore(genZhuangFen);

        let gangFen = huInfo["gangFen"] || 0;
        ShowNode.getChildByName('gangfenLb').getComponent("cc.Label").string = "杠分" + this.ToUiScore(gangFen);

        let maFen = huInfo["maFen"] || [];
        ShowNode.getChildByName('mafenLb').getComponent("cc.Label").string = "马分" + this.ToUiScore(maFen);;

        let lianZhuangStr = huInfo["lianZhuangShu"] > 0 ? "连庄数" + huInfo["lianZhuangShu"] : "";
		ShowNode.getChildByName('lianzhuangshuLb').active = lianZhuangStr;
    },
