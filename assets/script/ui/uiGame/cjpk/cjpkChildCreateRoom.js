...
var ddzChildCreateRoom = cc.Class({
    ...
    OnToggleClick: function (event) {
        ...
        if ('jushu' == key || 'renshu' == key || 'fangfei' == key) {
            ...
            if(toggleIndex==5 && !this.Toggles['fangjian'][5].getChildByName('checkmark').active){
    ...
    UpdateOnClickToggle: function () {
        if (this.Toggles["moshi"][0].getChildByName("checkmark").active) {
            ...
            this.Toggles['zhadanfenzhi'][1].parent.active = false;
        } else {
            this.Toggles['zhadanfenzhi'][1].parent.active = true;
        }
    },
    AdjustSendPack: function (sendPack) {
        if(sendPack.moshi == 0){
            sendPack.zhadanfenzhi = -1;
        }
        return sendPack;
    },
});
