...
var pypdkChildCreateRoom = cc.Class({
    ...
    CreateSendPack:function(renshu, setCount, isSpiltRoomCard){
        ...
        let jiesanfafen=-1;
        if (this.GetEditBoxByKey('jiesanfafen') === false) {
            app.SysNotifyManager().ShowSysMsg('请检查输入的区间值是否为0-20整数');
            return;
        }else{
            jiesanfafen=this.GetEditBoxByKey('jiesanfafen');
        }
        let kexuanwanfa = [];
        let fangjian = [];
    ...
    UpdateTogglesLabel:function (TogglesNode, isResetPos=true) {
            ...
            if(this.gameType == this.gameCreateConfig[key].GameName){
                if(curKey == this.gameCreateConfig[key].Key){
                    ...
                    if("kexuanwanfa" == curKey){
                        ...
                        this.Toggles['kexuanwanfa'][7].serverIdx = 30;//首出必带黑桃三
                        this.Toggles['kexuanwanfa'][8].serverIdx = 29;//更多分
                    }
        ...
        if (this.Toggles["gaoji"]) {
            for (let i = 0; i < this.Toggles["gaoji"].length; i++) {
                ...
                if (ToggleDesc == "30秒未准备自动踢出房间" || ToggleDesc == "10秒未准备自动踢出房间") {
