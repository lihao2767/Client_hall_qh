...
var hzzmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
        ...
        let qifan=this.GetIdxByKey('qifan');
        let kexuanwanfa=this.GetIdxsByKey('kexuanwanfa');
        let fengDing=-1;
        if (this.GetEditBoxByKey('fengDing') === false) {
            app.SysNotifyManager().ShowSysMsg('请检查输入的区间值是否为0-1000整数');
            return sendPack;
        }else{
            fengDing=this.GetEditBoxByKey('fengDing');
        }
    ...
    UpdateOnClickToggle: function() {
        //选项置灰
        if(this.Toggles["qihu"]){
            this.UpdateLabelColor(this.Toggles['qihu'][0].parent);
            if(this.Toggles["renshu"][2].getChildByName("checkmark").active){
                this.Toggles['qihu'][0].getChildByName("checkmark").active = true;
                this.Toggles['qihu'][1].getChildByName("checkmark").active = false;
                this.Toggles['qihu'][2].getChildByName("checkmark").active = false;
                //置灰
                if(this.Toggles['qihu'][0].getChildByName("label")){
                    this.Toggles['qihu'][0].getChildByName("label").color = cc.color(0, 155, 46);
                }
                if(this.Toggles['qihu'][1].getChildByName("label")){
                    this.Toggles['qihu'][1].getChildByName("label").color = cc.color(180, 180, 180);
                }
                if(this.Toggles['qihu'][2].getChildByName("label")){
                    this.Toggles['qihu'][2].getChildByName("label").color = cc.color(180, 180, 180);
                }
            }else if(this.Toggles["renshu"][1].getChildByName("checkmark").active){
                if(this.Toggles['qihu'][0].getChildByName("checkmark").active){
                    this.Toggles['qihu'][0].getChildByName("checkmark").active = false;
                    this.Toggles['qihu'][1].getChildByName("checkmark").active = true;
                    this.Toggles['qihu'][2].getChildByName("checkmark").active = false;
                    if(this.Toggles['qihu'][1].getChildByName("label")){
                        this.Toggles['qihu'][1].getChildByName("label").color = cc.color(0, 155, 46);
                    }
                    if(this.Toggles['qihu'][2].getChildByName("label")){
                        this.Toggles['qihu'][2].getChildByName("label").color = cc.color(158, 49, 16);
                    }
                }
                //置灰
                if(this.Toggles['qihu'][0].getChildByName("label")){
                    this.Toggles['qihu'][0].getChildByName("label").color = cc.color(180, 180, 180);
                }
                
            }else{
                this.Toggles['qihu'][0].getChildByName("checkmark").active = false;
                this.Toggles['qihu'][1].getChildByName("checkmark").active = false;
                this.Toggles['qihu'][2].getChildByName("checkmark").active = true;
                //置灰
                if(this.Toggles['qihu'][0].getChildByName("label")){
                    this.Toggles['qihu'][0].getChildByName("label").color = cc.color(180, 180, 180);
                }
                ...
                if(this.Toggles['qihu'][2].getChildByName("label")){
                    this.Toggles['qihu'][2].getChildByName("label").color = cc.color(0, 155, 46);
                }
            }
        }
    },
