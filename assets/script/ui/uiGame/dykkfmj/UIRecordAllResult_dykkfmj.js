...
cc.Class({
	...
	OnCreateInit: function () {
		...
		this.lastRoomId = 0;
		this.sp_restCard = this.node.getChildByName("sp_leftList");
		//加载麻将资源，防止异步坑爹的bug
		// this.LoadAllImages();
	},
	...
	InitShowPlayerInfo: function (setEnd) {
		...
		let restCardList = setEnd["leftTypeList"] || [];
	...
	ShowRestCard: function (restCardList) {
		...
		// }
		this.sp_restCard.active=false;
		let cardNode = this.node.getChildByName("cardDemo");
		let cardListNode = this.sp_restCard.getChildByName("mask").getChildByName("layout");
		cardListNode.removeAllChildren();
		for (let i = 0; i < restCardList.length; i++) {
			let childNode = cc.instantiate(cardNode);
			childNode.active = true;
			this.ShowImage(childNode, "EatCard_Self_", restCardList[i]);
	...
	OnClick: function (btnName, btnNode) {
		if (btnName == "btn_close") {
			...
			this.ShowData();
		} else if (btnName == "btn_rest") {
			this.sp_restCard.active = !this.sp_restCard.active;
		} else if (btnName == "btn_closeRest") {
			this.sp_restCard.active = false;
		} else {
