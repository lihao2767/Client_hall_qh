...
cc.Class({
	...
	ShowNiaoPai: function (cardIDList, zhongList) {
		...
		let cardNodeItem = this.node.getChildByName("card1");
		this.bg_niaopai.active = true;
		let niaopaiNode = this.bg_niaopai.getChildByName("scrollView").getChildByName("niaopai");
		niaopaiNode.removeAllChildren();
		for (let i = 0; i < cardIDList.length; i++) {
			let cardNode = cc.instantiate(cardNodeItem);
			niaopaiNode.addChild(cardNode);
			this.ShowImage(cardNode, cardIDList[i]);
			cardNode.active = true;
			cardNode.color = zhongList.indexOf(cardIDList[i]) != -1 ? cc.color(255, 255, 125) : cc.color(255, 255, 255);
		}
	},
