...
cc.Class({
	...
	UpdatePlayData: function (PlayerNode, HuList, PlayerInfo, jin1 = 0, jin2 = 0, jinJin = 0, zhuaNiaoList) {
		...
		this.ShowPlayerShowCard(PlayerNode.getChildByName('showcard'), HuList.shouCard, HuList.handCard, HuList.kouPais, jin1, jin2);
	...
	ShowOtherScore: function(ShowNode, huInfo) {
		...
		let hufen = huInfo["huPoint"] || 0;
        ShowNode.getChildByName('hufenLb').getComponent("cc.Label").string = "胡分:" + this.ToUiScore(hufen);

        let gangFen = huInfo["maPoint"] || 0;
        ShowNode.getChildByName('gangfenLb').getComponent("cc.Label").string = "马分:" + this.ToUiScore(gangFen);

        let maiDianPoint = huInfo["genZhuangPoint"] || 0;
        ShowNode.getChildByName('maifenLb').getComponent("cc.Label").string = "跟庄:" + this.ToUiScore(maiDianPoint);

        if (typeof (huInfo["chaJiaoPoint"]) != "undefined") {
			ShowNode.getChildByName('chajiaoLb').getComponent("cc.Label").string = "查叫分" + this.ToUiScore(huInfo["chaJiaoPoint"]);
		}else{
			ShowNode.getChildByName('chajiaoLb').getComponent("cc.Label").string = "";
		}
    },
	...
	IsShowMulti2: function (huType) {
		let multi2 = ["QYS","HYS","Long","ZWK","QD","HHQD", "CHHQD", "ZHHQD", "SSY", "ZhuangJia", "ZiMo", "DianPao", "KouPai", "KouDaJiang", "GSKH", "MQ", "HDLY", "PingHu"];		// 显示倍数胡类型
		let isShow = multi2.indexOf(huType) != -1;
		return isShow;
	},
...
//牌型分
			...
			SanHHQD: "三豪华七对",
			XSHHQD: "潇洒豪华七对",
			XSShuangHHQD: "潇洒双豪华七对",
			XSSanHHQD: "潇洒三豪华七对",
			XSHHQDDYG: "潇洒豪华七对带一滚",
			XSHHQDDEG: "潇洒豪华七对带二滚",
			XSHHQDDSG: "潇洒豪华七对带三滚",
			XSShuangHHQDDYG: "潇洒双豪华七对带一滚",
			XSShuangHHQDDEG: "潇洒双豪华七对带二滚",
			XSSanHHQDDYG: "潇洒三豪华七对带一滚",
			QXN: "七仙女",
			HQPP: "红旗飘飘",
			Long: "一条龙",
			QYS: "清一色",
...
//总分
    ...
    ShowPlayerShowCard: function (ShowNode, cardIDList, handCard, kouPais, jin1, jin2) {
        ShowNode.active = 1;
        let UICard_ShowCard = ShowNode.getComponent("UIMJCard_ShowCard");
        UICard_ShowCard.ShowDownCardBySWPMMJ(cardIDList, handCard, kouPais, jin1, jin2);
    },
});
