...
cc.Class({
    ...
    GetHuTypeDict: function () {
        ...
        huTypeDict["Mao"]="毛";
        huTypeDict["HS"]="胡数";
        huTypeDict["HuPoint"]="总分";
        huTypeDict["ChaTing"]="查听";
    ...
    PlayerData: function (PlayerNode, result, pos) {
        ...
        if (huType != 0) {
            ...
            for (let huType in huTypeMap) {
                ...
                // }
                if(huType == "PPH"){
                    lb_huInfo.getComponent(cc.Label).string = str + "x" + huPoint;
                }else{
        ...
        this.ShowHuCard(PlayerNode, result.huCard, result.tingCard);
    ...
    ShowHuCard: function (playerNode, huCard, tingCard) {
        let huNode = playerNode.getChildByName("huCardNode").getChildByName("huCard");
        if (!huNode) return;
        let lb_cardType = playerNode.getChildByName("huCardNode").getChildByName("lb_cardType");
        if(tingCard > 0){
            lb_cardType.getComponent(cc.Label).string = "听";
            huNode.cardID = tingCard;
        }else if (huCard > 0){
            lb_cardType.getComponent(cc.Label).string = "胡";
            huNode.cardID = huCard;
        }
        if (huCard <= 0 && tingCard <= 0){
            huNode.active = false;
            return;
        }
    ...
    ShowOutCardImage: function (childNode) {
        childNode.active = true;
        let imagePath = "ui/uiGame/zgsdr/cards/out/" + Math.floor(childNode.cardID / 100);
        let that = this;
