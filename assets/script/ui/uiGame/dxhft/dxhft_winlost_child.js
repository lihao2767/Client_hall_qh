...
cc.Class({
	...
	ShowPlayerShowCard: function (ShowNode, cardIDList, handCard, jin1, jin2) {
		...
		UICard_ShowCard.ShowDownCardByDXHFT(cardIDList, handCard, jin1, jin2);
	},
	...
	ShowPiaoMaiState: function (PlayerNode) {
		...
		let piaoFenStr = "";
		// if (this.posResultInfo["piao"] == 0) piaoFenStr = "不敲";
		// else if (this.posResultInfo["piao"] != -1) piaoFenStr = "敲";
		// // else if (this.posResultInfo["piao"] != -1) piaoFenStr = "漂" + this.posResultInfo["piao"];
		let piaoFen = this.posResultInfo["piao"];
		if (piaoFen == 0) {
			piaoFenStr = "不敲";
		} else if (piaoFen > 0) {
			piaoFenStr = "敲";
	...
	ShowPlayerJieSuan: function (ShowNode, huInfoAll) {
		...
		for (let huType in huInfo) {
			...
			if (this.IsShowMulti(huType)) {
				...
				let hustr = this.LabelName(huType) + "：" + huPoint;
				if (huType == "AnGang") {
					hustr = this.LabelName(huType) + (parseInt(huPoint) / 2) + "个数" + ":" + huPoint + "胡";
				} else if (huType == "PengHouGang") {
					hustr = this.LabelName(huType) + huPoint + "个数" + ":" + huPoint + "胡";
				}
				this.ShowLabelName(ShowNode.getChildByName("label_lists"), hustr);
			}
