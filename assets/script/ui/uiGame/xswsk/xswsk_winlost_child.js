...
cc.Class({
    ...
    properties: {
        ...
        bg_dadu: [cc.SpriteFrame],
        bg_endTypeTemp: [cc.SpriteFrame],
    },
    ...
    ShowPlayerData: function (setEnd, playerAll, index) {
        ...
        let challengePos = setEnd["challengePos"];
        ...
        let isRedWin = setEnd["isRedWin"];
        //
        let posEndList = posResultList[index];
        let ranksType = posEndList["ranksType"];

        let endTypeTemp = setEnd["endType"];
        if(isRedWin){
            if(endTypeTemp > -1 && ranksType == 1){
                this.node.getChildByName("user_info").getChildByName("img_type").getComponent(cc.Sprite).spriteFrame = this.bg_endTypeTemp[endTypeTemp - 1];
            }else{
                this.node.getChildByName("user_info").getChildByName("img_type").getComponent(cc.Sprite).spriteFrame = "";
            }
        }else{
            if(endTypeTemp > -1 && ranksType == 2){
                this.node.getChildByName("user_info").getChildByName("img_type").getComponent(cc.Sprite).spriteFrame = this.bg_endTypeTemp[endTypeTemp - 1];
            }else{
                this.node.getChildByName("user_info").getChildByName("img_type").getComponent(cc.Sprite).spriteFrame = "";
            }
        ...
        if(challengePos > -1){
            if(challengePos == index){
                this.node.getChildByName("user_info").getChildByName("img_huo").getComponent(cc.Sprite).spriteFrame = this.bg_dadu[0];
            }else{
                this.node.getChildByName("user_info").getChildByName("img_huo").getComponent(cc.Sprite).spriteFrame = "";
            }
        }else{
            if(posResultList[0].partnerPos != -1){
                if(index == 0 || index == posResultList[0].partnerPos){
                    this.node.getChildByName("user_info").getChildByName("img_huo").getComponent(cc.Sprite).spriteFrame = this.bg_dadu[1];
                }else{
                    this.node.getChildByName("user_info").getChildByName("img_huo").getComponent(cc.Sprite).spriteFrame = "";
                }
            }else{
