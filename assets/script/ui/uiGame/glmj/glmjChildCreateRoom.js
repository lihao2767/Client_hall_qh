...
var jsnyzmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
		let sendPack = {};
		let teshu = this.GetIdxByKey('teshu');
		let fanxing = this.GetIdxsByKey('fanxing');
		let xingwanfa = this.GetIdxsByKey('xingwanfa');
	...
	OnToggleClick: function (event) {
		...
		if ('jushu' == key || 'renshu' == key || 'fangfei' == key) {
			...
			if (this.Toggles["kexuanwanfa"]) {
				if (toggleIndex == 12 && !this.Toggles["kexuanwanfa"][12].getChildByName("checkmark").active) {
