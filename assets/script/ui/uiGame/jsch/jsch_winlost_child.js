...
cc.Class({
    ...
    ShowPlayerData: function (setEnd, playerAll, index) {
        ...
        // this.ShowRestCard(setEnd.restCard);
    },
    PlayerData: function (PlayerNode, result, pos) {
        ...
        for (let i = 0; i < cardPublicMap.length; i++) {
            ...
            let publicInfoValue = publicInfo["huShu"];

            let cardIDList = publicInfo["paiList"];

            let opType = publicInfo["paiXing"];
            if (opType == "shunZi") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "顺子";
            } else if (opType == "kou") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "口";
            } else if (opType == "peng") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "碰";
            } else if (opType == "keZi") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "克子";
            } else if (opType == "siZhang") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "四张";
            } else if (opType == "wuZhang") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "五张";
            } else if (opType == "liuZhang") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "六张";
            } else if (opType == "zhao") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "招";
            } else if (opType == "deng") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "登";
            } else if (opType == "di") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "抵";
            } else if (opType == "dui") {
                addYou.getChildByName("lb_paixing").getComponent(cc.Label).string = "对";
            } else {
                console.error("paiXing", opType);
            }
        ...
        lb_hushu.getComponent(cc.Label).string = "胡数:" + this.ToUiScore(result.allHuPoint);
        lb_fanshu.getComponent(cc.Label).string = "底分:" + this.ToUiScore(result.point);
        ...
        lb_haidilao.active = false;
    },
    ShowRestCard: function (restCard) {
        let restNode = this.node.getChildByName("dipai").getChildByName("resScrollView").getChildByName("view").getChildByName("layout");
        restNode.removeAllChildren();
        let demo_pai = this.node.getChildByName("demo_pai");
        demo_pai.x = 0;
        demo_pai.y = 0
        for (let i = 0; i < restCard.length; i++) {
            let addPai = cc.instantiate(demo_pai);
            addPai.cardID = restCard[i];
            restNode.addChild(addPai);
            this.ShowOutCardImage(addPai);
        }
    },
    ...
    ShowOutCardImage: function (childNode) {
        ...
        let imageName = ["zi_fang_", childNode.cardID].join("");
        if (childNode.cardID == 0) {
            imageName = ["zi_fang_bg"].join("");
        }
        if ([childNode.cardID].join("").length == 4) {
            imageName = ["zi_fang_", Math.floor(childNode.cardID / 100)].join("");
        }
