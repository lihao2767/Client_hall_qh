...
cc.Class({
    ...
    ShowPlayerData: function (setEnd, playerAll, index) {
        ...
        //炸弹分
        if(posEndList.bombScore>0){
            record.getChildByName('lb_bombfen').getComponent(cc.Label).string = "+" + posEndList.bombScore;
        }else{
            record.getChildByName('lb_bombfen').getComponent(cc.Label).string = posEndList.bombScore;
        }
        //底分
        record.getChildByName('lb_difen').getComponent(cc.Label).string = posEndList.diFen;
        if(posEndList.baseScore>0){
            record.getChildByName('lb_shuying').getComponent(cc.Label).string = "+" + posEndList.baseScore;
        }else{
            record.getChildByName('lb_shuying').getComponent(cc.Label).string = posEndList.baseScore;
        }
