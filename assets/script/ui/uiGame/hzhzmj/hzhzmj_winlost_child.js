...
cc.Class({
    ...
    ShowPlayerInfo: function (ShowNode, PlayerInfo, HuList) {
        ...
        let isLiang = HuList["isLiang"];

        ShowNode.getChildByName('ting').active = isTing;
        ShowNode.getChildByName('liang').active = isLiang;
    },
    ...
    ShowPlayerJieSuan: function (ShowNode, huInfoAll) {
        ...
        let zuoList = [];
        let youList = [];
        // huTypeMap = {FYS: 1, FS: 3, ZM: 1, PZG: 0}
        let huTypeMap = huInfo;
        for (let huType in huTypeMap) {
            let key = huType;
            let huTypeInfo = {};
            if (huType == "LZG") {
                huTypeInfo[key] = huTypeMap[huType];
                zuoList[0] = huTypeInfo;
            } else if (huType == "PZG") {
                huTypeInfo[key] = huTypeMap[huType];
                zuoList[1] = huTypeInfo;
            } else if (huType == "AG") {
                huTypeInfo[key] = huTypeMap[huType];
                zuoList[2] = huTypeInfo;
            } else if (huType == "FS") {
                huTypeInfo[key] = huTypeMap[huType];
                zuoList[3] = huTypeInfo;
            } else {
                huTypeInfo[key] = huTypeMap[huType];
                youList.push(huTypeInfo);
            }
        }
        let huTypeList = [].concat(zuoList, youList);
        // for (let huType in huTypeMap) {
        for (let i = 0; i < huTypeList.length; i++) {
            let huTypeInfo = huTypeList[i];
            for (let huType in huTypeInfo) {
                let huPoint = huTypeInfo[huType];
    ...
    ShowPlayerDownCard: function (ShowNode, publishcard, pengCardList, jin1 = 0, jin2 = 0) {
        ...
        UICard_DownCard.ShowDownCard(publishcard, pengCardList, jin1, jin2);
    },
});
