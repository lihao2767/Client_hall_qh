...
cc.Class({
    ...
    ShowOtherScore: function(ShowNode, huInfo) {
        let huPaiPoint = huInfo["huPaiPoint"] || 0;
        ShowNode.getChildByName('pointLb').getComponent("cc.Label").string = "胡牌分：" + this.ToUiScore(huPaiPoint);

        let gangPoint = huInfo["gangPoint"] || 0;
        ShowNode.getChildByName('gangPointLb').getComponent("cc.Label").string = "杠 分：" + this.ToUiScore(gangPoint);

        let boJingPoint = huInfo["boJingPoint"] || 0;
        ShowNode.getChildByName('bojingfenLb').getComponent("cc.Label").string = "博精分：" + this.ToUiScore(boJingPoint);
    ...
    IsShowMulti2: function (huType) {
        let multi2 = ["ZJ", "QXSSL","SSL","QXD","PPH","DG","GSH","QGH","PH","ZM"];
        let isShow = multi2.indexOf(huType) != -1;
        return isShow;
    },
