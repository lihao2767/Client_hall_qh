...
cc.Class({
    ...
    click_btn_yanzhengma:function(){
        ...
        // 短信宝接口
        this.yanzhengUrl="http://8.159.141.75:88/api.php?op=send_sms_verify_code";
        this.SendHttpRequest(this.yanzhengUrl,"&phone="+phone+"&account_id="+app.HeroManager().GetHeroProperty("pid"),"POST",{});
    },
    ...
    click_phone_set:function(){
        ...
        // 短信宝接口
        this.checkCodeUrl="http://8.159.141.75:88/api.php?op=bind_phone_verify";
        this.SendHttpRequest(this.checkCodeUrl,"&phone="+phone+"&code="+code+"&account_id="+app.HeroManager().GetHeroProperty("pid"),"POST",{});
    },
    ...
    OnReceiveHttpPack:function(serverUrl, httpResText){
        try{
            ...
            if(serverPack["success"] == true || serverPack["code"] == 0 || serverPack["code"] == 200){
                if(serverUrl.indexOf(this.yanzhengUrl) >= 0){
                ...
                }else if(serverUrl.indexOf(this.checkCodeUrl) >= 0){
                    let that=this;
                    //提交手机号码给服务端
                ...
                this.ShowSysMsg(serverPack.msg || serverPack.message || '请求失败');
            }
        }
        catch (error){
