...
cc.Class({
    ...
    OnShow: function (clubId,HeHuoRen=0,unionType=0) {
    	this.clubId=clubId;
        this.inviteCheck=app.ClubManager().GetClubInviteCheck(clubId);

        if(this.inviteCheck==1){
            //不需要
            this.node.getChildByName("btn_search").y=132;
        }else{
            this.node.getChildByName("btn_search").y=98;
        }
    ...
    ShowUser:function(data){
        ...
        usernode.getChildByName('name').getComponent(cc.Label).string=beiZhuName;
    ...
    click_btn_search:function(){
        ...
        if(isNaN(parseInt(shuru)) || !app.ComTool().StrIsNum(shuru)){
            app.SysNotifyManager().ShowSysMsg("请输入玩家ID", [], 3);
            return;
        }
    ...
    Click_btn_yaoqing:function(){
        ...
        if(this.HeHuoRen==0){
            ...
            let sendUrl="club.CClubFindPIDAdd";
            if(this.unionType==1){
                //中至模式
                sendUrl="club.CClubFindPIDAddZhongZhi";
            }
    		this.NetManager.SendPack(sendUrl, sendPack,function(success){
