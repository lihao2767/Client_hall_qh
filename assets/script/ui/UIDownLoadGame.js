...
cc.Class({
            ...
            SubgameManager.needUpdateSubgame(gameName, (success) => {
                if (success) {
                    ...
                    if (that.isEnterGame) {
                        ...
                        app.FormManager().ShowForm('UICreatRoomNew',{"gameList":app.Client.GetAllGameId()},gameName,clubDataTemp,that.unionData);
                        that.CloseForm();
                    }
    ...
    */
    CheckGameDown:function(){
        let checkUrl="http://8.159.141.75:82/"+this.selectGameName+"/remote-assets/version.manifest";
    ...
    EnterSubGameByType:function(name){
        ...
        if (that.isEnterGame) {
            ...
            app.FormManager().CloseForm('UITop');
            app.FormManager().GetFormComponentByFormName("UITop").RemoveCloseFormArr("UIMoreGame");
            app.FormManager().ShowForm('UICreatRoomNew',{"gameList":app.Client.GetAllGameId()},name,clubDataTemp,that.unionData);
            that.CloseForm();
            return;
        }
    },
