...
cc.Class({
    ...
    OnReceiveHttpPack:function(serverUrl, httpResText){
        try{
            ...
            if(serverUrl==this.CClubOnlinePlayerCount){
                if(serverPack["code"] == 0){
                   this.node.getChildByName("bottom").getChildByName("lb_OnlineCount").getComponent(cc.Label).string = "当前亲友圈在线人数："+serverPack.data+"人";
                }else{
                    app.SysNotifyManager().ShowSysMsg(serverPack['Msg'], [], 3);
                }
            }
        }
        catch (error){
    ...
    OnCreateInit:function(){
        ...
        this.CountServer = app.Client.GetClientConfigProperty("CountServer");
    ...
    OnShow:function(owerClubId,opClubId, unionId, unionName, unionSign){
        ...
        sendOnlinePack.exePid = app.HeroManager().GetHeroProperty("pid");
        let signString=app.HeroManager().GetHeroProperty("pid")+"wanzikeji"+app.ComTool().GetNowDateDayStr2();
        sendOnlinePack.sign= app.MD5.hex_md5(signString);

        this.CClubOnlinePlayerCount=this.CountServer+"/CClubOnlinePlayerCount";
        this.SendHttpRequest(this.CClubOnlinePlayerCount, "","POST",sendOnlinePack);
