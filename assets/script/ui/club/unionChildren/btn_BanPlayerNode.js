...
cc.Class({
    ...
    onLoad:function(){
        ...
        app.Client.RegEvent("OnUnionBanPlayerReShow", this.Event_OnUnionBanPlayerReShow, this);
    },
    ...
    GetPalyerList:function(isRefresh=false){
        ...
        let self = this;
        app.NetManager().SendPack("union.CUnionBanPlayerList",sendPack, function(serverPack){
    ...
    Event_OnUnionBanPlayerReShow:function(serverPack){
        this.curPage = 1;
        this.queryStr = "";
        this.node.getChildByName("searchEditBox").getComponent(cc.EditBox).string = "";
        //刷新页数
    ...
    OnClick:function(btnName, btnNode){
    	if('btn_next'==btnName){
            ...
            app.FormManager().ShowForm("ui/club/UIBanPlayerAddUser",this.unionId,this.clubId);
            ...
            let self = this;
            app.NetManager().SendPack("Union.CUnionBanPlayerDelete",sendPack, function(serverPack){
                let memberScrollView = self.node.getChildByName("memberScrollView");
                let content = memberScrollView.getChildByName("view").getChildByName("content");
                memberScrollView.getComponent(cc.ScrollView).scrollToTop();
                content.removeAllChildren();
                self.GetPalyerList(true);
                app.SysNotifyManager().ShowSysMsg("成功解除黑名单",[],3);
            }, function(){
                app.SysNotifyManager().ShowSysMsg("解除黑名单失败",[],3);
            });
        }
    },
    /**
