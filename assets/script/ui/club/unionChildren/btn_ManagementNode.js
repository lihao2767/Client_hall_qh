...
        var today = new Date();
    ...
    GetDataList:function(isRefresh=false){
        ...
        if (isRefresh) {
            ...
            app.NetManager().SendPack("union.CUnionRoomConfigPrizePoolCount",sendPack, function(serverPack){
                ...
                self.node.getChildByName("lb_kaifang_wuhao").getComponent(cc.Label).string = "总开房数(不含无消耗)："+serverPack.roomSizeConsumeNotZero;
    ...
    UpdateScrollView:function(serverPack, isRefresh){
    	...
    	for (let i = 0; i < serverPack.length; i++) {
            ...
            app.RoomCfgManager().WanFaFormServer(gameType,cfgObj,child.getChildByName("lb_roomCfg").getComponent(cc.Label),20,"比赛分设置："+this.GetUnionCfg(cfgObj),child);
