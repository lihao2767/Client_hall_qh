...
cc.Class({
    ...
    OnSchoolReporWinnerDetail:function(serverPack){
        let clubPlayerRoomAloneLogBOS=serverPack.clubPlayerRoomAloneLogBOS;
        let totalPageNum=serverPack.totalPageNum;
        this.FormManager.ShowForm('ui/club/UIClubWinerDetail',clubPlayerRoomAloneLogBOS);
    ...
    ShowRecordList:function(data, isRefresh){
        ...
        for(let i=0;i<data.length;i++){
            ...
            nodePrefab.getChildByName('btn_lb_yingjia').getComponent(cc.Label).string="大赢家："+playerData.winner+"场";

            nodePrefab.getChildByName('btn_lb_yingjia').pid=playerData['player'].pid;
	...
	OnClick:function(btnName, btnNode){
		if('btn_close'==btnName){
            ...
            this.recordPage--;
            this.GetClubRecord(true);
        }else if('btn_lb_yingjia'==btnName){
            let pid=btnNode.pid;

            let sendPack = {"clubId":this.clubId, "roomIDList":this.roomIDList, "isAll":this.isAll, "getType":this.getType, "pageNum":this.recordPage, "query":this.query,"pid":pid};
            if (this.unionId > 0 && this.isShowAllClub) {
                //显示所有亲友圈战绩
                sendPack.unionId = this.unionId;
            }
            let that=this;
            this.NetManager.SendPack("club.CClubSChoolReportDetailByPid",sendPack ,function(event){
               that.OnSchoolReporWinnerDetail(event);
            },function(error){
                console.log(error);
            });
