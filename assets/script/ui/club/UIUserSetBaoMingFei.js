...
cc.Class({
    ...
    */
    OnShow: function (data) {
        ...
        lb_TargetPercent.string = "<color=#705d52>修改</c><color=#f8772c>" + data.name.substr(0,9) + "（ID:" + data.pid + "）" + "</color><color=#705d52>的活跃系数</c>";
        ...
        if(data.shareType==1){
            //固定值
            lb_curPercent.string = "该玩家当前活跃系数：" + data.shareFixedValue;
            ...
            lb_curPercent.string = "该玩家当前活跃系数：" + data.shareValue+"%";
            //初始化比赛分输入框
        ...
        if(btnName == "btn_sure"){
                ...
                if (parseFloat(percentStr) >= 0) {
                    ...
                    app.NetManager().SendPack("club.CClubPromotionShareChange",sendPack, function(serverPack){
                        app.SysNotifyManager().ShowSysMsg("成功设置活跃系数",[],3);
                        //触发刷新上层
                        app.Client.OnEvent('UpdateBaoMingFeiFenCheng', serverPack);
                        self.CloseForm();
                    }, function(){
                        // app.SysNotifyManager().ShowSysMsg("设置活跃系数失败",[],3);
                    });
