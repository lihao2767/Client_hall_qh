...
cc.Class({
    ...
    ShowData: function(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            //玩家 - 使用缓存的解析结果避免重复JSON.parse导致卡顿
            if(!parsedCache[record.roomKey]){
                parsedCache[record.roomKey] = {
                    datainfo: JSON.parse(record.dataJsonRes),
                    playerList: JSON.parse(record.playerList)
                };
            }
            let datainfo = parsedCache[record.roomKey].datainfo;
            let playerList = parsedCache[record.roomKey].playerList;
    ...
    OnClick_ShowMore: function(btnNode) {
        ...
        if (this.ShareDefine.GameType_QYYBZMJ == record.gameType) {
            let smallName = this.ShareDefine.GametTypeID2PinYin[record.gameType];
            let path = "ui/uiGame/" + smallName + "/UIRecordAllResult_" + smallName;
            this.FormManager.ShowForm(path, record.roomId, JSON.parse(record.playerList), record.gameType, this.unionId);
            return;
