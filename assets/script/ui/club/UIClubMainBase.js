...
var BaseClubMainForm = cc.Class({
    ...
    OnCreateInit: function () {
        ...
        this.RegEvent("HideStatusOpen", this.Event_HideStatusOpen, this);
    ...
    OnShow: function () {
        ...
        this.schedule(this.RunRoomDisplay, 1);
    ...
    //添加游戏玩法
    LoadGameWanFa: function (unionId, clubId) {
        if (unionId > 0) {
            app.NetManager().SendPack('union.CUnionGameList', { "unionId": unionId }, function (data) {
                app.RoomCfgManager().GetWanFaFormServerBatchAdd(data.split(","));

            });
        } else if (clubId > 0) {
            app.NetManager().SendPack('club.CClubGameList', { "clubId": clubId }, function (data) {
                app.RoomCfgManager().GetWanFaFormServerBatchAdd(data.split(","));
            });
        }
    },
    Event_LoadClub: function (clubData) {
        ...
        console.log("收到亲友圈信息：" + new Date().getTime());
        ...
        */
        ...
        if (this.myisminister != app.ClubManager().Club_MINISTER_GENERAL) {
            ...
            if (this.myisminister == app.ClubManager().Club_MINISTER_CREATER || this.myisminister == app.ClubManager().Club_MINISTER_MGRSS) {
                this.node.getChildByName('top').getChildByName('right_btn').getChildByName('btn_union').active = false;  //圈主也不显示
            } else {
    ...
    Event_RoomPlayerChange: function (serverPack) {
        ...
        for (let i = 0; i < this.roomList.length; i++) {
            if (serverPack.roomKey == this.roomList[i].roomKey) {
                ...
                if (serverPack['pos'].oldPosId > -1) {
                    //清空旧的玩家
                    this.roomList[i].posList[serverPack['pos'].oldPosId].pid = 0;
                }
    ...
    ShowRoomDataBatch: function (batchRoomKey) {
        ...
        app.NetManager().SendPack(packName, { 'clubId': this.nowClubID, 'unionId': this.nowUnionID, 'roomKey': batchRoomKey }, function (serverPack) {
            console.log("收到房间详细信息：" + new Date().getTime());
            ...
            if (playerNum <= 6) {
                if (ClubTb == 5) {
                    ...
                    if (isPuke == false) {
                        ...
                        club.getChildByName("bg").getComponent(cc.Sprite).spriteFrame = this.roomBg_zhongzhi_green[playerNum + 1];
                    }
                } else if (ClubTb == 6) {
                    //蓝色
                    if (isPuke == false) {
                        ...
                        club.getChildByName("bg").getComponent(cc.Sprite).spriteFrame = this.roomBg_zhongzhi_blue[playerNum + 1];
                    }
                }
            }
    ...
    GetZindex: function (sort, tagId, roomkey) {
        if (!cc.sys.isNative) {
            this.sort = 3;  //网页版连接显示测试排序使用
        }
        let fudongzhi = parseInt(roomkey.substring(0, 2));
        let zIndexNum = 255;
        if (this.sort == 0) {
            //已开-人满-有人-空桌
            if (sort == 0) {   //开始的
                zIndexNum = tagId * 100 + fudongzhi;  //zIndex不能等于0，不然一定傻逼到天
            } else if (sort == 1) {                   //人满
                zIndexNum = (tagId + 80) * 100 + fudongzhi;
            } else if (sort == 2) {             //未满
                zIndexNum = (160 + tagId) * 100 + fudongzhi;
            } else if (sort == 3) {             //空桌
                zIndexNum = (240 + tagId) * 100 + fudongzhi;
            }
            return zIndexNum;
        }
        if (this.sort == 1) {
            //空桌固定序号 未开-人满-已开（默认） 
            if (sort == 0) {
                zIndexNum = (200 + tagId) * 100 + fudongzhi;//开始的
            } else if (sort == 1) {
                zIndexNum = (100 + tagId) * 100 + fudongzhi;//人满
            } else if (sort == 2) {             //未满人
                zIndexNum = tagId * 100 + fudongzhi;
            } else if (sort == 3) {          //空配置
                zIndexNum = tagId * 100 + fudongzhi;
            }
            return zIndexNum;
        ...
        if (this.sort == 2) {
            //排序 未开-已开-空桌
            if (sort == 0) {  //开始的
                zIndexNum = (150 + tagId) * 100 + fudongzhi;
            } else if (sort == 1) {                  //人满
                zIndexNum = tagId * 100 + fudongzhi;
        ...
        if (this.sort == 3) {
            ...
            if (sort == 0) {  //开始的
                ...
                zIndexNum = (80 + tagId) * 100 + fudongzhi;
            }
            return zIndexNum;
        }
        return tagId;
    ...
    SortAllRoom: function (childRoom, roomData) {
        ...
        childRoom.zIndex = this.GetZindex(roomData.sort, roomData.tagId, roomData.roomKey);
    ...
    Click_btn_qhwh: function () {
        let isSelect = this.node.getChildByName("left_wanfa").active;
        if (isSelect == false) {
            let sendPack = {};
    ...
    UpdateLeftWanFa: function (serverPack) {
        ...
        for (let i = 0; i < serverPack.length; i++) {
            ...
            if (serverPack[i].setCount > 100 && serverPack[i].setCount < 200) {
                ...
                //阳阳改，修复筛选错误
                let roomData = serverPack[i];  //roomData 是报错的。添加个读取来解决  this.roomList
                //这个是改的if(this.ShareDefine.GametTypeID2PinYin[serverPack[i]] == "yzyzmj"){
                if (this.ShareDefine.GametTypeID2PinYin[roomData.gameId] == "yzyzmj") {
                    //jushuStr="第"+serverPack[i].setId+"/"+serverPack[i].setCount%100+"圈";
                }
            }
