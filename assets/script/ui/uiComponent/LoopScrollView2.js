...
var LoopScrollView = cc.Class({
    ...
    RefreshData:function(dataList, isRefresh){
        if (isRefresh) {
            this.totalDataList = dataList;
        }else{
            this.totalDataList = this.totalDataList.concat(dataList);
        }
        
        let totalCount = this.totalDataList.length;

        let haveCreateCount = this.allChildNodeList.length;
        ...
        if(this.scrollDir == "vertical"){
            //设置滑动控件的最大滑动高度
            let totalHeight = createCount * (this.itemHeight + this.Spacing) + this.Spacing;
        ...
        for(let index=haveCreateCount; index<createCount; index++){
            ...
            this.CreateScrollViewChild(index, userData, pos);
        }

        this.ScrollView.stopAutoScroll();

        if(haveCreateCount >= createCount){
            if(this.scrollDir == "vertical"){
    ...
    //初始化滑动数据
    InitScrollData:function(parentNode,childPrefab, dataList, callback, itemHeight=0){
        //上次更新的content滑动Y坐标 
        this.lastContentPosY = 0;
        this.lastContentPosX = 0;
        this.allChildNodeList = [];
        ...
        if(this.scrollDir == "vertical"){
            ...
            if(itemHeight){
                ...
                this.itemHeight = this.childPrefab.height;
                if(!this.itemHeight){
            ...
            if(itemHeight){
                ...
                this.itemWidth = this.childPrefab.width;
                if(!this.itemWidth){
        ...
        this.itemAnchorX = this.childPrefab.anchorX;
        this.itemAnchorY = this.childPrefab.anchorY;
     ...
     */
    CreateScrollViewChild:function(index, userData=0, pos=null, zorderLv=0){
        let itemNode = cc.instantiate(this.childPrefab);
        this.parentNode.addChild(itemNode);
        itemNode.setPosition(pos);
        itemNode.index = index;
        itemNode.zIndex = index;
        this.callback(index, itemNode, userData);
        this.allChildNodeList.push(itemNode);
    },
    ...
    OnCheckVerticalReusability:function(){
        ...
        let childCount = this.allChildNodeList.length;
        ...
        if (this.ScrollView.node.anchorY == 0) {
            limitDown = 0;
            limitUp = this.viewHeight;
        }else if (this.ScrollView.node.anchorY == 1) {
            limitDown = -this.viewHeight;
            limitUp = 0;
        } 
        //创建的子控件总长度之和
        ...
        for (let index = 0; index < childCount; index++) {
            let childFormNode = this.allChildNodeList[index];
            let viewPos = this.GetPositionInView(childFormNode);
            ...
            if(isDown) {
                ...
                if (viewPos.y + this.itemHeight*(1-this.itemAnchorY) < limitDown){
                    ...
                    let newPosY = childFormNode.y - limitDown - this.itemHeight;
                    // this.ErrLog("Down (%s) newPosY:%s,%s", index, newPosY, childFormNode.y);

                    //如果计算出的新坐标已经在顶层可见了,则设置子界面从底层到顶层
                    if(newPosY < 0){
                        let childIndex = childFormNode.zIndex;

                        let newChildIndex = childIndex - childCount;
                        let userData = this.totalDataList[newChildIndex];
                        
                        if(userData){
                            this.ErrLog("Down childIndex(%s)=>(%s,%s)", childIndex, newChildIndex, userData);
                            this.callback(newChildIndex, childFormNode, userData);
                            // //在底部创建一个空节点，保证节点数量跟数据量一致
                            // let emptyNode = new cc.Node();
                            // emptyNode.height = this.itemHeight;
                            // this.parentNode.addChild(emptyNode);
                            // emptyNode.zIndex = childIndex;
                            // this.allChildNodeList.push(emptyNode);
                        }
                        else{
                            // this.ErrLog("Down childIndex(%s)=>(%s,%s)", childIndex, newChildIndex, userData);
                        }
                ...
                if (viewPos.y - this.itemHeight*this.itemAnchorY > limitUp){
                    ...
                    if(newPosY > -this.ScrollView.content.height){
                        ...
                        if(userData){
                            this.ErrLog("Up childIndex(%s)=>(%s,%s)", childIndex, newChildIndex, userData);
                            this.callback(newChildIndex, childFormNode, userData);
                            // //在顶部部创建一个空节点，保证节点数量跟数据量一致
                            // let emptyNode = new cc.Node();
                            // emptyNode.height = this.itemHeight;
                            // this.parentNode.addChild(emptyNode);
                            // emptyNode.zIndex = childIndex;
