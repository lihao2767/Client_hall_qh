...
var NetManager = app.BaseClass.extend({
	...
	OnCodeError: function (eventName, code, resultInfo) {
		...
		// }
			...
			app.SysNotifyManager().ShowSysMsg("您未满18周岁不允许游戏", [], 3);
		}
		else if (code == 105) {
			app.SysNotifyManager().ShowSysMsg("城市未开放");
		}
		else if (code == 914) {
			app.SysNotifyManager().ShowSysMsg('NotEnough_IdCardAgeValid');
		}
		else if (code == 5029) {
			app.SysNotifyManager().ShowSysMsg('不在房间内');
		}
		else if (code == 6161) {
			app.SysNotifyManager().ShowSysMsg('该玩法已被关闭');
		}
		else if (code == 915) {
			app.SysNotifyManager().ShowSysMsg('NotEnough_IdCardAgeNoValid');
			...
			app.SysNotifyManager().ShowSysMsg("修改后新上级推广员系数比例低于当前玩家比例，无法修改，请先修改玩家或新上级推广员的比例，使新上级推广员系数大于玩家系数");
		}
		else if (code == 916) {
			app.SysNotifyManager().ShowSysMsg("尊敬的各位用户:自12月6日00:00起，本游戏关服1天。预计12月7日00:00后恢复，以游戏内实际开服时间为准。");
		}
			...
			app.SysNotifyManager().ShowSysMsg('已邀请或未找到该玩家或同赛事不同亲友圈不能重复拉人或距离退出该亲友圈不到10分钟或该玩家处于赛事黑名单，无法加入亲友圈');
		}
