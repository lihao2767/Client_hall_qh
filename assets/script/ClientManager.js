...
cc.Class({
	...
	OnPack_OnHeroLogin: function (serverPack) {
		...
		try {
			...
			//this.GetAllGameIdFromServer();  //界面OnShow的时候有请求过一次了
			//初始化实时语音
			console.log("初始化实时语音");
	...
	GetBaseGameServerIP:function(){
		return this.clientConfig['GameServerIP'];
	},
	GetAppName: function () {
	...
	GetGameListByCityId:function(cityId){
		let self = this;
		//根据选择的城市向服务端请求所有游戏
		app.NetManager().SendPack("game.CGetGameListByCityId", {"selectCityId": cityId}, function (event) {
					let curGameList = event.split(",");
					let argDict = {
						"gameList": curGameList,
					};
					self.allGameId=curGameList;
					cc.sys.localStorage.setItem("myCityID",cityId);
					app.HeroManager().UpdateCity(cityId);
					if(curGameList.length>0){
						self.OnEvent("ShowGameListByLocation", argDict);
						app.RoomCfgManager().GetWanFaFormServerBatch(curGameList); //获取改城市的游戏玩法
					}
			}, function (event) {
	...
	GetAllGameIdFromServer: function () {
		...
		if (allSelectCityData.length == 0) {
			...
			this.GetGameListByCityId(selectId);
		}
	},

	GetAllGameId: function () {
		if(!this.allGameId){
			return {};
		}
		return this.allGameId;
	},
	...
	OnConFirm: function (clickType, msgID, backArgList) {
		...
		if (msgID == "RESELECT_CITY") {
			...
			this.GetGameListByCityId(backArgList[0]);
