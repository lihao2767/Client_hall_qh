...
var HeroAccountManager = app.BaseClass.extend({
    ...
    Init: function () {
        ...
        this.NetManager.RegHttpNetPack(0x0014, this.OnHttpPack_SendLoginPhoneVerifyCodeResult, this);
    ...
    UpdateAccessPointForce: function () {
        console.log("UpdateAccessPointForce in");
        let now = new Date().getTime();
        if (now - this.lastChangeTime < 5000) {
            console.log("UpdateAccessPointForce in 1");
            return;
        }
        this.lastChangeTime = now;
        //获取旧的节点能打开不，能打开就不换节点
        console.log("UpdateAccessPointForce in 2");
    ...
    UpdateAccessPoint: function () {
        ...
        this.lastChangeTime = now;
        //获取旧的节点能打开不，能打开就不换节点（已注释，不访问checklive.php）
    ...
    //注册账号结果
    //result:0-注册成功(新账号) 1-账号已存在(失败) 其它-其它失败
    OnHttpPack_RegAccountResult: function (serverPack) {
        let result = serverPack.Result;
        //取出注册时设置的回调(若有),执行完立即清空,避免重复触发
        let callback = this.RegAccountCallBack;
        this.RegAccountCallBack = null;
    ...
    //注册账号
    //callback: 可选,注册结果回调,参数为布尔值(true=成功,false=失败)
    SendRegAccount: function (charAccount, accountPsw, callback) {
        if (this.isDoLogin) {
        ...
        //保存回调,等注册结果回来时触发
        this.RegAccountCallBack = callback || null;
    ...
    SendLoginPhoneVerifyCode: function (phone, callback) {
        console.log("=== SendLoginPhoneVerifyCode called ===");
        console.log("phone:", phone);
        this.SendLoginVerifyCodeCallBack = callback || null;
        let sendPack = this.NetManager.GetHttpSendPack(0xFF14);
        console.log("sendPack:", sendPack);
        if (!sendPack) {
            console.error("GetHttpSendPack(0xFF14) returned null!");
            return;
        }
        sendPack.PhoneNum = phone;
        this.NetManager.SendHttpPack(sendPack);
        console.log("SendHttpPack called");
    },

    OnHttpPack_SendLoginPhoneVerifyCodeResult: function (serverPack) {
        let result = serverPack.Result;
        let msg = serverPack.Msg;
        let callback = this.SendLoginVerifyCodeCallBack;
        this.SendLoginVerifyCodeCallBack = null;
        if (callback) {
