...
cc.Class({
    ...
    //加载
    onLoad: function () {
        // 强制设置为奉新版（登录界面用湖北版，游戏大厅用奉新版）
        cc.sys.localStorage.setItem("appName", "xinhua");
        console.log("===== 强制设置 appName = xinhua（奉新版）=====");
        console.log("当前 appName:", cc.sys.localStorage.getItem('appName'));
        //节点更新回默认节点
        //app.LocalDataManager().SetConfigProperty("Account", "AccessPoint",0);
        //cc.sys.localStorage.setItem("AccessPoint",0);
        //客户端启动初始状态
        ...
        if (appName=="hubei") {
            this.bgDefault.active=false;
            this.bgHuBei.active=true;
        }else{
            this.bgDefault.active=true;
            this.bgHuBei.active=false;
        }
        app["tanchuang"]=1;
